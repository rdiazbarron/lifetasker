  ...
  return (
    <main className="mx-auto max-w-6xl p-6 text-slate-100 space-y-6">
      <div className="flex items-end justify-between">
        ...
        <p className="text-sm text-slate-400">Weekly-flex progress overview</p>
      </div>
      {error && (
      ...
      <section className="rounded-lg border border-slate-700 p-4">
        ...
        <p className="text-sm text-slate-400 mb-3">
          Log finished blocks and instantly refresh weekly progress.
        </p>
        {blockTypes.length === 0 ? (
          <p className="rounded bg-slate-800/60 p-3 text-slate-300">
            No block types yet. Create one in Block Types first.
          </p>
        ) : (
          <ul className="space-y-2">
            {blockTypes.map((bt) => (
              <li
                key={bt.id}
                className="flex items-center justify-between rounded bg-slate-800/40 p-2"
              >
                <span>{bt.name}</span>
                <CompleteBlockButton
                  onClick={async () => {
                    await api.completeBlock({ blockTypeId: bt.id });
                    setStatus(`Completed one ${bt.name} block.`);
                    await load();
                  }}
                />
              </li>
